...
router.get('/get', (req, res) =>{   
    ...
    Collection.find({userId:userId}).then((post,err)=>{        
        if (post.length) {
            ...
            console.log(err)
            res.send('Коллекции отсутствуют')
          }
    })
})

router.get('/getall', (req, res) =>{
  Collection.find().then((post,err)=>{      
      if (post.length) {
          res.send({out:post})
        } 
        else {
...
router.put('/update-collection', (req, res) =>{
    const collectionId = req.body.params.data.collectionId
    Collection.updateOne({_id: collectionId}, 
      {
...
router.get('/getuserid',(req,res)=>{
  ...
  Collection.find({_id:collectionId}).then((post,err)=>{
    if (post.length){
      ...
      res.send({out:post[0].userId})
    }
    else{
      console.log(err)
      res.send('По вашему запросу ничего не найдено.')
    }
  })
})
