...
import {getItem} from '../actions/item-func'
import  ShowOneItem  from '../items/component/showOneItem'
import {withRouter} from 'react-router-dom'
import jwt_decode from 'jwt-decode'
import { getUserId } from '../actions/collection-func'

class PageItemCollection extends Component {
  constructor(props) {
      super(props)
      this.state={
        titlecomp:'Список айтемов этой коллекции:',
        items:[]
      }
...
async onSubmit(e){
 ...
 show = async () => {   
      if(this.mounted){
      getItem(localStorage.getItem('collectionID')).then(res=>{
        console.log(res)
        this.setState({
          items: res.data.out
        }) 
...
export default withRouter(PageItemCollection)